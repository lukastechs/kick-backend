...
async function fetchUserPublicV1({ userId, token }) {
  ...
  try {
    ...
    console.error('Public v1 /users error:', {
      status: error.response?.status,
      data: error.response?.data,
      message: error.message,
    });
...
async function handleKickRequest(req, res) {
  ...
  try {
    ...
    if (!resolvedSlug && broadcasterUserId) {
      v1Channel = await fetchChannelPublicV1({ slug: null, broadcasterUserId, token });
      if (v1Channel && v1Channel.slug) {
        resolvedSlug = v1Channel.slug.toLowerCase();
      } else {
        v1User = await fetchUserPublicV1({ userId: broadcasterUserId, token });
        if (v1User && v1User.id) {
          resolvedSlug = v1User.username?.toLowerCase();
        }
      }
      if (!resolvedSlug) {
    ...
    if (!v1Channel) {
      v1Channel = await fetchChannelPublicV1({ slug: resolvedSlug, broadcasterUserId: null, token });
    }
    if (!v1User && (v2?.user_id || v1Channel?.user_id)) {
      v1User = await fetchUserPublicV1({ userId: v2?.user_id || v1Channel?.user_id, token });
    }

    // If both v1 and v2 failed, bail out
    if (!v2 && !v1Channel && !v1User) {
      return res.status(404).json({ error: `User ${resolvedSlug} not found` });
    }

    // Extract fields, prioritizing v2 for accuracy
    const createdAt = v2?.chatroom?.created_at || v1Channel?.created_at || null;
    const ageDays = calculateAgeDays(createdAt);

    const data = {
      // Identifiers
      slug: resolvedSlug,
      channel_id: v2?.id ?? v1Channel?.id ?? null,
      user_id: v2?.user_id ?? v1Channel?.user_id ?? v1User?.id ?? null,

      // Core display fields
      username: v2?.user?.username ?? v1Channel?.user?.username ?? v1User?.username ?? resolvedSlug,
      description: v2?.user?.bio ?? v1User?.bio ?? v1Channel?.channel_description ?? 'N/A',
      avatar: v2?.user?.profile_pic ?? v1User?.profile_pic ?? v1Channel?.profilepic ?? 'https://via.placeholder.com/50',

      // Status & metrics
      verified: v2?.verified === true ? 'Yes' : (v1User?.verified ? 'Yes' : (v1Channel?.verified ? 'Yes' : 'No')),
      followers: typeof v2?.followers_count === 'number' ? v2.followers_count : (v1Channel?.followers_count || v1User?.followers_count || 0),
      is_banned: v2?.is_banned === true ? 'Yes' : 'No',

      // Live + category
      category: v1Channel?.category?.name ?? (Array.isArray(v2?.recent_categories) && v2.recent_categories.length ? v2.recent_categories[0].name : 'N/A'),
      is_live: typeof v1Channel?.stream?.is_live === 'boolean' ? (v1Channel.stream.is_live ? 'Yes' : 'No') : (Boolean(v2?.livestream) ? 'Yes' : 'No'),
      stream_title: v1Channel?.stream_title ?? v2?.livestream?.session_title ?? 'N/A',
